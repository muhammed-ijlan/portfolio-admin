...
export default function ProjectView() {
  ...
  const getProject = async () => {
    try {
      setIsLoading(true);
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/project/one`, { params: { projectId: id } });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const res = await axios.put(`${process.env.REACT_APP_API_URL}/project`, data);
