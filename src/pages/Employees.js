...
// import Filter from '../components/filters/EmployeeFilter';
...
export default function Employee() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      try {
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/employee/admin/create`, values, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        if (!res.data.isError) {
          closeAddEmployeeModal(res.data.message);
        } else {
  ...
  const getAllDesignations = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/configuration/admin/designations`);
      if (!res.data.isError) {
  ...
  const closeAddEmployeeModal = (message) => {
    setOpen(false);
    resetForm();
    toast(message);
  };
  ...
  const getEmployees = (options) => {
    setIsLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_URL}/employee/admin/all`, {
        params: options,
      })
      .then((res) => {
        console.log('res', res);
              ...
              <Card>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                  ...
                  <Stack padding={1} direction="row" justifyContent="flex-end" spacing={2} alignItems="center">
                    <Button startIcon={<AddIcon />} variant="contained" onClick={openAddEmployeeModal}>
                      Add Employee
                    </Button>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {employeeData?.map((item, id) => (
                            ...
                            <TableCell>
                              <Box>
                                <Link to={`/dashboard/Employee/${item._id}`}>
                                  <Icon sx={{ color: 'gray' }}>
                                    <RemoveRedEyeIcon />
                                  </Icon>
                                </Link>
